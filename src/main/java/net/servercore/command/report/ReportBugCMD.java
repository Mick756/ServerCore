package net.servercore.command.report;

import lombok.SneakyThrows;
import net.servercore.ACommand;
import net.servercore.ServerCore;
import net.servercore.command.prompts.BugReportPrompt;
...
public class ReportBugCMD extends ACommand {
	...
	public ReportBugCMD() {
		...
		reportFile = new File(ServerCore.getInstance().getDataFolder(), "bug_reports.yml");
		reports = YamlConfiguration.loadConfiguration(reportFile);
		
		cooldown = TimeUnit.SECONDS.toMillis(ServerCore.getInstance().getConfig().getLong("command-settings.bug-report.cooldown"));
	...
	public void execute(CommandSender sender, String label, String[] args) {
		...
		if (args.length == 0) {
			...
			ConversationFactory conF = new ConversationFactory(ServerCore.getInstance());
