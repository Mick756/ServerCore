...
public class Payment {
	...
	public boolean completePayment() {
		Economy economy = ServerCore.getEconomy();
	...
	public void message() {
		this.sender.sendMessage(ServerCore.getPrefix() + String.format(Globals.color("&7You successfully paid&b %s&a $%.2f&7."), this.receiver.getName(), this.amount));
		if (this.receiver.isOnline() && this.receiver.getPlayer() != null) {
			this.receiver.getPlayer().sendMessage(ServerCore.getPrefix() + String.format(Globals.color("&7You received a payment from&b %s&7 of &a $%.2f&7."), this.sender.getName(), this.amount));
		}
	}
	
	@SneakyThrows
	public void log() {
		PrintWriter writer = new PrintWriter(new File(ServerCore.getInstance().getDataFolder(), "pay_logs.txt"));
