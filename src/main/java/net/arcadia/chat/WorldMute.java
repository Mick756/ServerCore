...
public class WorldMute extends Mute {
	...
	public WorldMute(String world, long duration, String description) {
		this.world = world;
	...
	public List<Arcadian> getMutedPlayers() {
		...
		if (world != null) {
			arcadians.addAll(world.getPlayers().stream().map(Arcadian::get).collect(Collectors.toList()));
		}
