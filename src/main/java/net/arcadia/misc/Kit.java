...
public class Kit {
	...
	public Kit(String name, long cooldown, ItemStack[] items) {
		...
		this.file = new File(ArcadiaCore.getInstance().getDataFolder() + "/kits", name + ".yml");
		if (!this.file.exists()) {
	...
	public void give(Player player) {
		...
		if (timeLeft != null) {
			...
			arcadian.sendMessage(true, "&cYou must wait&e %s&c more before using this kit again.", Util.toReadableTime(timeLeft));
			return;
		}
		
		List<Integer> emptySlots = arcadian.getEmptySlots();
		if (emptySlots.size() < this.items.length) {
			arcadian.sendMessage(true, "&cYou do not have enough empty slots to receive this kit.");
			return;
		}
		
		player.getInventory().addItem(this.items);
		arcadian.sendMessage(true, "&7You have received the kit&b %s&7.", this.name);
	...
	public static int loadKits() {
		...
		File folder = new File(ArcadiaCore.getInstance().getDataFolder() + "/kits");
			...
			if (kitFiles != null && kitFiles.length > 0) {
				...
				for (File kitFile : kitFiles) {
					...
					if (items != null) {
						...
						ArcadiaCore.error("&cThe kit '" + name + "' failed to load.");
					}
				}
